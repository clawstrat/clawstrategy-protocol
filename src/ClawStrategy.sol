...
contract ClawStrategy is Ownable, Pausable, ReentrancyGuard {
    ...
    /// @notice Adopt an existing Clanker token that was deployed outside ClawStrategy
    ...
    function adoptToken(
        ...
        if (tokenConfigs[token].agent != address(0)) revert TokenAlreadyExists();
        ...
        IClankerLpLocker.TokenRewardInfo memory rewardInfo = IClankerLpLocker(locker).tokenRewards(token);
